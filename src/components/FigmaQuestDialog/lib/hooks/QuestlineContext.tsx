...
export function QuestlineProvider({ children, initialQuestKeys = [] }: QuestlineProviderProps) {
  ...
  const initialQuestStates = initialQuestKeys.reduce<Record<string, QuestState>>((acc, questKey) => {
    acc[questKey] = 'locked'
    return acc
  }, {})
  ...
  const resetAllStates = useCallback(() => {
    setState((prev) => ({
      ...prev,
      questStates: Object.keys(prev.questStates).reduce<Record<string, QuestState>>((acc, questKey) => {
        acc[questKey] = 'locked'
        return acc
      }, {}),
      headerState: 'active',
  ...
  return <QuestlineContext.Provider value={contextValue}>{children}</QuestlineContext.Provider>
}
